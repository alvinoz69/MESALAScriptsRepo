...
DROP PROCEDURE IF EXISTS MIGRATION_DEF_ACCOUNTS
...
CREATE PROCEDURE dbo.MIGRATION_DEF_ACCOUNTS
	@HEADER_ID BIGINT,
	@BUSINESS_DATE DATE,
	@IS_COMPLETED INT = 0,
	@SP_NAME NVARCHAR(50) = 'MIGRATION_DEF_ACCOUNTS',
		...
		BEGIN TRY
			...
			--->INSERT INTO SELECT STATEMENT
				...
				INSERT INTO CBDB_REPORTS.RPT.DEFICIENT_ACCOUNTS WITH(TABLOCK) 
					...
					SELECT @HEADER_ID ,ACM.ACCOUNT_NO ,@BUSINESS_DATE ,0    ,ISNULL(ACM.TOTAL_OVERDUE,0) ,ISNULL(ACM.LOAN_BALANCE,0) ,GETDATE()    ,1
				FROM CBDB_REPORTS.LMS.ACCOUNT_MASTER ACM WITH(NOLOCK)
				WHERE ACM.IS_CLOSE = 0
				AND   ACM.ACCOUNT_STATUS_CODE <> '01'
				AND   ACM.LOAN_BALANCE > 0
			--	AND   ACM.TOTAL_OVERDUE > 0 FOR CLARIFICATION
				SET @INSERTED_RECORDS = @INSERTED_RECORDS + @@ROWCOUNT

				--1st Collection Letter
				INSERT INTO CBDB_REPORTS.RPT.DEFICIENT_ACCOUNTS WITH(TABLOCK) 
						  (HEADER_ID  ,ACCOUNT_NO     ,BUSINESS_DATE  ,CODE ,BALANCE                     ,LOAN_BALANCE               ,DATE_CREATED ,IS_MIGRATED)
					SELECT @HEADER_ID ,ACM.ACCOUNT_NO ,@BUSINESS_DATE ,1    ,ISNULL(ACM.TOTAL_OVERDUE,0) ,ISNULL(ACM.LOAN_BALANCE,0) ,GETDATE()    ,1
				FROM CBDB_REPORTS.LMS.ACCOUNT_MASTER ACM WITH(NOLOCK)
				WHERE ACM.IS_CLOSE = 0
				AND   ACM.ACCOUNT_STATUS_CODE <> '01'
				AND   ACM.LOAN_BALANCE > 0
			--	AND   ACM.TOTAL_OVERDUE > 0 FOR CLARIFICATION
				...
				INSERT INTO CBDB_REPORTS.RPT.DEFICIENT_ACCOUNTS WITH(TABLOCK) 
						  (HEADER_ID  ,ACCOUNT_NO     ,BUSINESS_DATE  ,CODE ,BALANCE                     ,LOAN_BALANCE               ,DATE_CREATED ,IS_MIGRATED)
					SELECT @HEADER_ID ,ACM.ACCOUNT_NO ,@BUSINESS_DATE ,2    ,ISNULL(ACM.TOTAL_OVERDUE,0) ,ISNULL(ACM.LOAN_BALANCE,0) ,GETDATE()    ,1
				FROM CBDB_REPORTS.LMS.ACCOUNT_MASTER ACM WITH(NOLOCK)
				WHERE ACM.IS_CLOSE = 0
				AND   ACM.ACCOUNT_STATUS_CODE <> '01'
				AND   ACM.LOAN_BALANCE > 0
			--	AND   ACM.TOTAL_OVERDUE > 0 FOR CLARIFICATION
				...
				INSERT INTO CBDB_REPORTS.RPT.DEFICIENT_ACCOUNTS WITH(TABLOCK) 
						  (HEADER_ID  ,ACCOUNT_NO     ,BUSINESS_DATE  ,CODE ,BALANCE                     ,LOAN_BALANCE               ,DATE_CREATED ,IS_MIGRATED)
					SELECT @HEADER_ID ,ACM.ACCOUNT_NO ,@BUSINESS_DATE ,3    ,ISNULL(ACM.TOTAL_OVERDUE,0) ,ISNULL(ACM.LOAN_BALANCE,0) ,GETDATE()    ,1
				FROM CBDB_REPORTS.LMS.ACCOUNT_MASTER ACM WITH(NOLOCK)
				WHERE ACM.IS_CLOSE = 0
				AND   ACM.ACCOUNT_STATUS_CODE <> '01'
				AND   ACM.LOAN_BALANCE > 0
